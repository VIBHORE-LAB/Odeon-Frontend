...
const TopArtistCard: React.FC<TopArtistCardProps> = ({ artist }) => {
  return (
    <div className="bg-gradient-to-b from-gray-900 to-black text-white rounded-2xl p-4 shadow-md hover:shadow-green-500/30 transform hover:-translate-y-1 hover:scale-105 transition-all duration-300 relative group">
      {/* Artist Image */}
      {artist.images?.[0] && (
        <div className="overflow-hidden rounded-xl mb-3">
          <img
            ...
            alt={artist.name}
            className="w-full h-48 object-cover rounded-xl transform group-hover:scale-110 transition-transform duration-300"
          />
        </div>
      )}
